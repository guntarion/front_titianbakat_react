...
const QuizTypeA = ({ quizId, scoreTypes, onQuizComplete, initialQuestionIndex = 0, calculateScores }) => {
    const { user } = useAuth()
    ...
    useEffect(() => {
        const fetchQuizData = async () => {
            try {
                ...
                // Fetch quiz metadata
                const metadataResponse = await axios.get(`${config.API_URL}/quizzes/${quizId}`)

                // Fetch quiz statements and other details
                const statementsResponse = await axios.get(`${config.API_URL}/quiz/${quizId}`)
    ...
    const handleAnswerClick = index => {
        ...
        if (currentQuestionIndex < quizData.quiz_statements.length - 1) {
            ...
            const totalScores = calculateScores(newResponses, quizData.statementTypeMap)
    ...
    return (
        <div className="quiz-container">
            <p>
                Anda dapat mengerjakan quiz ini dalam beberapa sesi. Jika Anda meninggalkan halaman ini, maka saat Anda kembali lagi, Anda dapat langsung melanjutkan tanpa memulai
                dari awal.
            </p>
            ...
            <div className="question-container">
                ...
                <input type="text" value={noteText} onChange={e => setNoteText(e.target.value)} placeholder="Catatan Anda..." className="mt-3 answer-input" />
            </div>
            <p style={{ textAlign: "center" }}>
                {currentQuestionIndex + 1} dari {quizData.quiz_statements.length} pertanyaan terlampaui.
            </p>
        </div>
    )
}
