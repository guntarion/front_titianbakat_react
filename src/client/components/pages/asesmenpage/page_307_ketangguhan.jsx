// src/client/components/pages/asesmenpage/page_307_ketangguhan.jsx
import React, { useState, useEffect } from 'react';
import { Link, useHistory } from 'react-router-dom';
import Header from '../../header';
import QuizTypeA from '../../Quiz/quiz-type-a';
import QuizResult from '../../Quiz/Quiz-Result/result_307_ketangguhan';
...
const Asesment307Ketangguhan = (props) => {
  const { user } = useAuth();
    ...
    const fetchData = async () => {
      ...
      if (docSnap.exists()) {
        ...
        if (data.mongoUserId) {
          try {
            // Fetch quiz info
            const quizInfoResponse = await axios.get(`${config.API_URL}/quizzes/quiz_307_ketangguhan`);
            setQuizInfo(quizInfoResponse.data);

            // Fetch latest in-progress quiz response
            try {
              const latestInProgressResponse = await axios.get(`${config.API_URL}/quiz-responses/${data.mongoUserId}/quiz_307_ketangguhan/latest-in-progress`);
            ...
            try {
              const latestFinishedResponse = await axios.get(`${config.API_URL}/quiz-responses/${data.mongoUserId}/quiz_307_ketangguhan/latest-finished`);
...
const renderQuizStatusMessage = () => {
  ...
  const startQuiz = async () => {
    try {
      ...
      const url = `${config.API_URL}/quiz-responses/${mongoUserId}/quiz_307_ketangguhan/latest`;
...
export default Asesment307Ketangguhan;