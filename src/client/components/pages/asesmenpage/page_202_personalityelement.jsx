// src/client/components/pages/asesmenpage/page_202_personalityelement.jsx
import React, { useState, useEffect } from 'react';
import { Link, useHistory } from 'react-router-dom';
import Header from '../../header';
import QuizTypeA from '../../Quiz/quiz-type-a';
import QuizResult from '../../Quiz/Quiz-Result/result_202_personalityelement';
...
const Asesment202PersonalityElement = (props) => {
  const { user } = useAuth();
    ...
    const fetchData = async () => {
      ...
      if (docSnap.exists()) {
        ...
        if (data.mongoUserId) {
          try {
            // Fetch quiz info
            const quizInfoResponse = await axios.get(`${config.API_URL}/quizzes/quiz_202_personalityelement`);
            setQuizInfo(quizInfoResponse.data);

            // Fetch latest in-progress quiz response
            try {
              const latestInProgressResponse = await axios.get(`${config.API_URL}/quiz-responses/${data.mongoUserId}/quiz_202_personalityelement/latest-in-progress`);
            ...
            try {
              const latestFinishedResponse = await axios.get(`${config.API_URL}/quiz-responses/${data.mongoUserId}/quiz_202_personalityelement/latest-finished`);
...
const renderQuizStatusMessage = () => {
  ...
  const startQuiz = async () => {
    try {
      ...
      const url = `${config.API_URL}/quiz-responses/${mongoUserId}/quiz_202_personalityelement/latest`;
              ...
              <div className="container-fluid">
                ...
                <div className="col-12">
                  {showLatestResult ? (
                    ...
                    {showQuiz && (
                      <QuizTypeA
                        quizId="quiz_202_personalityelement"
...
export default Asesment202PersonalityElement;