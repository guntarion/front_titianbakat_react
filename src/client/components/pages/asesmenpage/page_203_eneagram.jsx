// src/client/components/pages/asesmenpage/page_203_eneagram.jsx
import React, { useState, useEffect } from 'react';
import { Link, useHistory } from 'react-router-dom';
import Header from '../../header';
import QuizTypeA from '../../Quiz/quiz-type-a';
import QuizResult from '../../Quiz/Quiz-Result/result_203_eneagram';
...
const Asesment203Eneagram = (props) => {
  const { user } = useAuth();
  ...
  const scoreTypes = ["Type_1_Reformer", "Type_2_Helper", "Type_3_Achiever", "Type_4_Individualist", "Type_5_Investigator", "Type_6_Loyalist", "Type_7_Enthusiast", "Type_8_Challenger", "Type_9_Peacemaker"];
    ...
    const fetchData = async () => {
      ...
      if (docSnap.exists()) {
        ...
        if (data.mongoUserId) {
          try {
            // Fetch quiz info
            const quizInfoResponse = await axios.get(`${config.API_URL}/quizzes/quiz_203_eneagram`);
            setQuizInfo(quizInfoResponse.data);

            // Fetch latest in-progress quiz response
            try {
              const latestInProgressResponse = await axios.get(`${config.API_URL}/quiz-responses/${data.mongoUserId}/quiz_203_eneagram/latest-in-progress`);
            ...
            try {
              const latestFinishedResponse = await axios.get(`${config.API_URL}/quiz-responses/${data.mongoUserId}/quiz_203_eneagram/latest-finished`);
...
const renderQuizStatusMessage = () => {
  ...
  const startQuiz = async () => {
    try {
      ...
      const url = `${config.API_URL}/quiz-responses/${mongoUserId}/quiz_203_eneagram/latest`;
...
export default Asesment203Eneagram;