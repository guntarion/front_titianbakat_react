          ...
          if (data.mongoUserId) {
            try {
              const response = await axios.get(
                `${config.API_URL}/users/${data.mongoUserId}`
              );
              const mongoData = response.data;
              setUserInfo({
                namaLengkap: mongoData.namaLengkap || '',
                tanggalLahir: mongoData.tanggalLahir
                  ? new Date(mongoData.tanggalLahir)
                  : null,
            ...
            <div className='patient-details'>
              {userInfo.tanggalLahir && (
                <h5>
                  ...
                  {formatBirthDate(userInfo.tanggalLahir)},{' '}
                  {calculateAge(userInfo.tanggalLahir)} years
                </h5>
              )}
